...
DataList util::mergeOutputMulti(Semantics *sem, DataStorage &&all_inp, const std::vector<ExecuteInfo *>& info_list) {
...
namespace {
    void _printNode(FTANode* node) {
        ...
        LOG(INFO) << "Node#" << node->getInfo() << " " << node->symbol->name << "@" << node->size << " " << data::dataList2String(oup_list);
        for (auto* edge: node->edge_list) {
            auto edge_string = edge->semantics->getName();
            for (auto* child: edge->node_list) edge_string += " (" + std::to_string(child->getInfo()) + ")";
...
void util::verifyFTAEdge(FTANode *node, FTAEdge *edge, const std::vector<ExecuteInfo*>& info_list) {
    DataStorage child_outputs;
    for (auto* child: edge->node_list) child_outputs.push_back(child->oup_info->getFullOutput());
    auto expected_output = util::mergeOutputMulti(edge->semantics.get(), std::move(child_outputs), info_list);
    if (!(node->oup_info->getFullOutput() == expected_output)) {
        LOG(FATAL) << "Fail when verifying " << node->toString() << " -> " << edge->toString() << ", get unexpected output " << data::dataList2String(expected_output);
    }
}

bool util::isEmpty(FTA* fta) {
    return !fta || fta->isEmpty();
}
...
FTASizeInfo size::getCompressVector(FTA *fta, FTA *base) {
    assert(fta->size_limit == base->size_limit && fta->grammar == base->grammar);
...
std::string size::sizeInfo2String(const FTASizeInfo &info) {
    ...
    for (int i = 0; i < info.size(); ++i) {
        if (i) res += ", ";
        if (fabs(info[i] - int(info[i])) < 1e-8 && info[i] >= 0 && info[i] < 1e9) {
...
PFTA util::cloneFTA(FTA *fta) {
    ...
    for (int size = 1; size <= fta->size_limit; ++size) {
        for (auto& [_, storage]: fta->node_map) {
            for (auto* node: storage[size]) {
                new_node_list[node->getInfo()] = new FTANode(node->symbol, node->oup_info, node->size);
            }
        }
    }
        ...
        for (auto& [_, storage]: fta->node_map) {
            for (auto* node: storage[size]) {
                ...
                for (auto* edge: node->edge_list) {
                    ...
                    auto* new_edge = new FTAEdge(edge->semantics, std::move(new_children));
    ...
    return std::make_shared<FTA>(fta->grammar, fta->size_limit, holder, root_list, fta->info_list);
}