...
struct CombineInfo {
    ...
    CombineInfo() = default;
    CombineInfo(FixedVector<FTANode *> &&_others,
                FixedVector<FTANode *> &&_children)
        : others(std::move(_others)), children(std::move(_children)) {}
    CombineInfo(CombineInfo &&other)
...
FTANode *merge::util::createNode(FTANode *base,
                                 const FixedVector<FTANode *> &others) {
...
FTAEdge *merge::util::createEdge(const PSemantics &semantics,
                                 FixedVector<FTANode *> &&children) {
...
std::vector<ExecuteInfo *> merge::util::mergeInfoList(const FTAList &fta_list) {
    ...
    auto share_num =
        extractSharedInfo(base->root_list[0])->getFullOutput().size();
...
namespace {
bool _stepOnTrie(const FixedVector<FTANode *> &others,
                 const FixedVector<TrieNode *> &node_list,
                 FixedVector<TrieNode *> &result) {
#ifdef DEBUG
...
void _mergeForward(int index, const std::vector<SplitHolder *> &info_holder,
                   const FixedVector<TrieNode *> &trie_nodes,
                   SplitHolder &result, FixedVector<FTANode *> &children,
...
void _naiveStepOnTrie(const FixedVector<FTANode *> &others,
                      const FixedVector<TrieNode *> &node_list,
                      FixedVector<TrieNode *> &result) {
#ifdef DEBUG
    assert(others.size() == node_list.size() && others.size() == result.size());
...
void _mergeForward(const std::vector<SplitHolder *> &info_holder,
                   const FixedVector<TrieNode *> &roots, SplitHolder &res,
                   const PSemantics &sem) {
...
void fta::merge::util::forwardStep(
    FTANode *base, const std::vector<trie::FTAEdgeHolder> &edge_holders,
    ...
    for (auto *base_edge : base->edge_list) {
        auto feature =
            trie::getEdgeFeature(base->symbol, base_edge->semantics.get());
        bool is_root_available = true;
        for (int i = 0; i < edge_holders.size(); ++i) {
            auto it = edge_holders[i].find(feature);
            if (it == edge_holders[i].end()) {
                is_root_available = false;
                break;
            } else {
                trie_roots[i] = it->second.get();
            }
...
PFTA fta::merge::mergeFTAForwardMulti(const FTAList &fta_list, bool is_cut) {
    ...
    for (int i = 0; i < fta_list.size(); ++i)
        index_list[i] = {-fta_list[i]->nodeCount(), i};
    ...
    for (int size = 1; size <= base->size_limit; ++size) {
        for (auto &[_, storage] : base->node_map) {
            for (auto *base_node : storage[size]) {
                ...
                forwardStep(base_node, edge_holders, holder_list,
                            holder_list[INDEX(base_node)]);
                // LOG(INFO) << "end forward " <<
                // holder_list[INDEX(base_node)].map.size();
            }
        }
    }
...
PFTA fta::merge::mergeFTAForwardMultiCart(const FTAList &fta_list,
                                          bool is_cut) {
#ifdef DEBUG
    checkShared(fta_list);
#endif
    ...
    auto holder_list = util::initSplitHolderList(base, n);
    for (int size = 1; size <= base->size_limit; ++size) {
        for (auto &[symbol_name, nodes] : base->node_map) {
            FTANodeStorage node_storage;
            for (auto fta : other_list) {
                node_storage.push_back(fta->node_map[symbol_name][size]);
            }
            auto all_node_combination = cartesianProduct(node_storage);
            for (auto *node : nodes[size]) {
                auto &result_parent = holder_list[INDEX(node)];
                for (auto *edge : node->edge_list) {
                    int child_num = edge->node_list.size();
                    for (auto &node_combination : all_node_combination) {
                        auto *res_node =
                            result_parent.query(node_combination.clone());
                        std::vector<std::vector<FTAEdge *>> edge_storage;
                        for (auto *other_node : node_combination) {
                            // filter edges with same semantics
                            std::vector<FTAEdge *> edge_list;
                            for (auto *other_edge : other_node->edge_list) {
                                if (edge->semantics == other_edge->semantics)
                                    edge_list.push_back(other_edge);
                            }
                            edge_storage.push_back(edge_list);
                        }
                        for (auto &edge_combination :
                             cartesianProduct(edge_storage)) {
                            FixedVector<FTANode *> children(child_num);
                            for (int child_ind = 0; child_ind < child_num;
                                 child_ind++) {
                                FixedVector<FTANode *> child_parts(
                                    edge_combination.size());
                                auto &result_child = holder_list[INDEX(
                                    edge->node_list[child_ind])];
                                for (int i = 0; i < edge_combination.size();
                                     i++)
                                    child_parts[i] = edge_combination[i]
                                                         ->node_list[child_ind];
                                children[child_ind] =
                                    result_child.query(std::move(child_parts));
                            }
                            auto *new_edge = createEdge(edge->semantics,
                                                        std::move(children));
                            res_node->edge_list.push_back(new_edge);
                        }
                    }
...
void _checkShare(FTA *base, FTA *fta) {
    assert(base->grammar == fta->grammar &&
           base->size_limit == fta->size_limit);
    assert(!base->root_list.empty() && !fta->root_list.empty());
    auto *root_x = base->root_list[0];
    auto *root_y = fta->root_list[0];
    assert(extractSharedInfo(root_x)->getFullOutput() ==
           extractSharedInfo(root_y)->getFullOutput());
}
        ...
        for (int size = 0; size < fta_list[0]->size_limit; ++size) {
            ...
            for (auto &fta : fta_list) {
                for (auto *node : fta->node_map[symbol->name][size]) {
                    ...
                    auto feature =
                        data::dataList2String(shared_info->getFullOutput());
...
EdgeFeature::EdgeFeature(FTAEdge *edge)
    : sem(edge->semantics), info_list(edge->node_list.size()) {
...
void EdgeGroup::init(FTANode *_node) {
    ...
    std::sort(
        ...
        [&](EdgeInfo *x, EdgeInfo *y) { return cmp(x->first, y->first); });

    for (int now = 0, next = 0; now < edge_infos.size(); now = next) {
        next = now + 1;
        while (
            next < edge_infos.size() &&
            cmp.isEqual(edge_infos[next]->first, edge_infos[next - 1]->first))
            ++next;
        FixedVector<FTAEdge *> edges(next - now);
        for (int i = now; i < next; ++i) edges[i - now] = edge_infos[i]->second;
        edge_groups.emplace_back(std::move(edge_infos[now]->first),
                                 std::move(edges));
    }
}
...
fta::merge::util::backward::mergeEdgeGroup(
    const FixedVector<EdgeGroup *> &group_list) {
    ...
    typedef std::vector<
        std::pair<EdgeFeature, FixedVector<FTAEdge *>>>::iterator Iterator;
    int n = group_list.size();
    FixedVector<Iterator> pointers(n);
    for (int i = 0; i < group_list.size(); ++i)
        pointers[i] = group_list[i]->edge_groups.begin();
    EdgeFeatureCmp cmp;
    ...
    while (!is_finished) {
        ...
        same_count = 1;
        for (int i = 1; i < n; ++i) {
            auto &sign = (sign_list[i] = cmp.getSign(pointers[index]->first,
                                                     pointers[i]->first));
            if (sign == 1) {
                sign = 0;
                index = i;
        ...
        if (same_count == n) {
            ...
            for (int i = 0; i < n; ++i)
                storage[i] = pointers[i]->second.clone();
...
namespace {
void _initMergedRoots(int index, const FTANodeStorage &storage,
                      FixedVector<FTANode *> &tmp, SplitHolder &holder,
                      FTANodeList &roots) {
...
FTANodeList fta::merge::util::backward::initRoots(
    FTA *base, const FTAList &others, SplitHolderList &holder_list) {
    FTANodeList root_list;
    for (auto *base_root : base->root_list) {
        ...
        for (auto &fta : others) {
            ...
            for (auto *root : fta->root_list) {
                if (root->symbol == base_root->symbol &&
                    root->size == base_root->size) {
                    root_list.push_back(root);
                }
            }
        }
        FixedVector<FTANode *> tmp(others.size());
        _initMergedRoots(0, storage, tmp, holder_list[INDEX(base_root)],
                         root_list);
    }
    return root_list;
}
...
namespace {
void _combineEdge(int index, const EdgeStorage &storage,
    ...
    if (index == storage.size()) {
        ...
        for (int child_index = 0; child_index < holder_list.size();
             ++child_index) {
            ...
            children[child_index] =
                holder_list[child_index]->query(std::move(node_list));
        }
...
SmallVector<FixedVector<FTANode *>> _combineEdge(const EdgeStorage &storage,
    ...
    for (auto *first_edge : storage[0]) {
        FixedVector<SplitHolder *> base_holder_list(
            first_edge->node_list.size());
...
void fta::merge::util::backwardStep(
    FTANode *base,
    ...
    for (auto &[others, merged_node] : start.map) {
        ...
        for (auto &[sem, edge_storage] :
             util::backward::mergeEdgeGroup(group_list)) {
...
PFTA fta::merge::util::createRawFTA(
    FTA *base, const SplitHolderList &holder_list, const FTANodeList &root_list,
    ...
    return std::make_shared<FTA>(base->grammar, base->size_limit, node_holder,
                                 root_list, info_list);
}

PFTA fta::merge::mergeFTABackwardMultiWithShare(const FTAList &fta_list,
                                                bool is_cut) {
#ifdef DEBUG
    checkShared(fta_list);
#endif
    ...
    auto fta =
        util::createRawFTA(base, holder_list, roots, mergeInfoList(fta_list));
    if (is_cut) fta->fullyCut();
    return fta;
}

PFTA fta::merge::mergeFTABackwardMulti(const FTAList &fta_list, bool is_cut,
                                       int share_num) {
#ifdef DEBUG
