...
namespace {
PProgram rawCEGISAfter(Specification* spec, PFTA fta, MergeType type,
                       int size) {
    ...
    while (!fta::util::isEmpty(fta.get())) {
        ...
        ++global::example_num;
        LOG(INFO) << "NEW EXAMPLE RAW"
                  << example::ioExample2String(counter_example);
        if (fta::util::isEmpty(new_fta.get())) break;
        fta = fta::merge::mergeFTAForward(new_fta.get(), fta.get(), false,
                                          nullptr);
...
PProgram KFoldCEGISAfter(Specification* spec, PFTA fta, MergeType type,
                         int size) {
    IOExampleList counter_examples;
    IOExample counter_example;
    auto* env = spec->env.get();
...
PProgram fta::synthesis::rawCEGIS(Specification* spec, MergeType type,
    ...
    for (int size = start_size; size; ++size) {
        ...
        auto res = rawCEGISAfter(spec, fta, type, size);
        if (res) return res;
    }
    assert(0);
}
...
PProgram _kFoldCEGIS(Specification* spec, int fold_num,
    ...
    auto res = KFoldCEGISAfter(spec, fta, type, size);
...
PFTA _rawMerge(PFTA fta, const IOExampleList& example_list, Env* env,
    ...
    for (auto& example : example_list) {
        ...
        if (util::isEmpty(fta.get()) || util::isEmpty(start.get()))
            return nullptr;
        global::recorder.start("multi");
...
PFTA _rawMergeCart(PFTA fta, const IOExampleList& example_list, Env* env,
                   MergeType type) {
    auto* grammar = fta->grammar;
    int size = fta->size_limit;
    for (auto& example : example_list) {
        ...
        if (util::isEmpty(fta.get()) || util::isEmpty(start.get()))
            return nullptr;
        global::recorder.start("multi");
        fta = mergeFTACart(fta.get(), start.get(), type, false);
        global::node_count += fta->nodeCount();
        global::edge_count += fta->edgeCount();
        // LOG(INFO) << "current " << fta->getSizeInfo();
        fta->fullyCut();
...
// old merge
PFTA fta::synthesis::rawMergeFast(Specification* spec,
...
// new merge
/*PFTA fta::synthesis::rawMerge(Specification *spec, const IOExampleList
&example_list, MergeType type, int size) { auto* grammar =
spec->info_list[0]->grammar; auto* env = spec->env.get(); auto fta =
grammar2FTA(grammar, size, true); for (auto& example: example_list) { auto start
= buildFTA(grammar, env, example, size, true);
        //assert(!util::isEmpty(fta.get()) && !util::isEmpty(start.get()));
        if(util::isEmpty(fta.get()) || util::isEmpty(start.get())) return
nullptr; _addEmptyLeft({fta, start}); fta = merge::mergeFTAForwardMulti({fta,
start}, false); fta->fullyCut();
    }
    return fta;
}*/
// new merge 2
PFTA fta::synthesis::rawMerge(Specification* spec,
    ...
    auto fta = grammar2FTA(grammar, size, true);
    for (auto& example : example_list) {
        // LOG(INFO) << "new example" << '\n';
        auto start = buildFTA(grammar, env, example, size, true);
        // assert(!util::isEmpty(fta.get()) && !util::isEmpty(start.get()));
        if (util::isEmpty(fta.get()) || util::isEmpty(start.get()))
            return nullptr;
        //_addEmptyLeft({fta, start});
        // fta = merge::mergeFTAForwardMulti({fta, start}, false);
        if (type == fta::FORWARD)
            fta =
                merge::mergeFTAForward(start.get(), fta.get(), false, nullptr);
        else
            fta =
                merge::mergeFTABackward(start.get(), fta.get(), false, nullptr);
        LOG(INFO) << "current " << fta->getSizeInfo();
        global::node_count += fta->nodeCount();
        global::edge_count += fta->edgeCount();
        global::fta_size.push_back(fta->nodeCount());
        fta->fullyCut();
    }
    return fta;
}

PFTA fta::synthesis::allMerge(Specification* spec,
                              const IOExampleList& example_list, MergeType type,
                              int size) {
    auto* grammar = spec->info_list[0]->grammar;
    auto* env = spec->env.get();
    ...
    for (auto& example : example_list) {
        auto start = buildFTA(grammar, env, example, size, true);
        // assert(!util::isEmpty(fta.get()) && !util::isEmpty(start.get()));
        if (util::isEmpty(start.get())) return nullptr;
        global::node_count += fta->nodeCount();
        global::edge_count += fta->edgeCount();
        merge_list.push_back(start);
    }
    _addEmptyLeft(merge_list);
    fta = merge::mergeFTAForwardMulti(merge_list, false);
    global::node_count += fta->nodeCount();
    global::edge_count += fta->edgeCount();
    fta->fullyCut();
    return fta;
...
PFTA fta::synthesis::multiMerge(Specification* spec,
                                const IOExampleList& example_list, int fold_num,
                                MergeType type, int size) {
    auto* grammar = spec->info_list[0]->grammar;
    auto* env = spec->env.get();
    ...
    for (int i = 0; i < fold_num; i++) {
        auto fta = grammar2FTA(grammar, size, true);
        merge_list.push_back(fta);
    }
    int cur_idx = 0;
    for (auto& example : example_list) {
        auto start = buildFTA(grammar, env, example, size, true);
        // assert(!util::isEmpty(fta.get()) && !util::isEmpty(start.get()));
        merge_list[cur_idx] = merge::mergeFTAForward(
            start.get(), merge_list[cur_idx].get(), false, nullptr);

        global::node_count += merge_list[cur_idx]->nodeCount();
        global::edge_count += merge_list[cur_idx]->edgeCount();
        merge_list[cur_idx]->fullyCut();
        cur_idx = (cur_idx + 1) % fold_num;
    }
    _addEmptyLeft(merge_list);
    auto fta = merge::mergeFTAForwardMulti(merge_list, false);
    global::node_count += fta->nodeCount();
    global::edge_count += fta->edgeCount();
    fta->fullyCut();
    return fta;
...
PFTA fta::synthesis::kFoldMerge(Specification* spec,
                                const IOExampleList& example_list, int fold_num,
    ...
    auto fio_space =
        example::buildFiniteIOExampleSpace(example_list, func_name, env);
    auto new_spec =
        std::make_shared<Specification>(spec->info_list, spec->env, fio_space);
    auto init = grammar2FTA(spec->info_list[0]->grammar, size, true);
    scheduler->start(init.get());
    global::recorder.start("pre");
    auto fta_list =
        kfold::kFold(new_spec.get(), fold_num, scheduler, type, size);
    global::recorder.end("pre");
...
PFTA fta::synthesis::kFoldMergeCart(Specification* spec,
                                    const IOExampleList& example_list,
                                    int fold_num,
                                    MultiMergeScheduler* scheduler,
                                    MergeType type, int size) {
    ...
    global::recorder.start("multi");
    if (type == fta::FORWARD)
        full = fta::merge::mergeFTAForwardMultiCart(fta_list, false);
    else
        full = fta::merge::mergeFTABackwardMultiWithShare(fta_list, false);
    LOG(INFO) << "Raw " << full->getSizeInfo();
    LOG(INFO) << "  "
    ...
    return _rawMergeCart(full, remaining_examples, spec->env.get(), FORWARD);
}
...
PFTA fta::synthesis::bidMerge(Specification* spec,
                              const IOExampleList& example_list,
                              BidConfig config, int size_limit) {
    IOExampleList init_examples;
    IOExampleList other_examples;
    ...
    auto unit_list =
        kfold::prepareUnits(spec, init_examples, config.shared_num, size_limit);
    assert(!unit_list.empty());

    auto fta =
        fta::merge::mergeFTAStagedBidirectional(unit_list, true, config.alpha);
...
PProgram fta::synthesis::bidCEGIS(Specification* spec, BidConfig config) {
    IOExampleList init_examples = _getRandomExample(
        spec->example_space.get(), spec->env.get(), config.init_example_num);
    auto* grammar = spec->info_list[0]->grammar;
    int size_limit = grammar::getMaxSize(grammar);
    for (int size = 1;; ++size) {
        LOG(INFO) << "Size " << size;
        if (size_limit != -1 && size > size_limit) return nullptr;
        auto unit_list = fta::kfold::prepareUnits(spec, init_examples,
                                                  config.shared_num, size);
        ...
        auto res = rawCEGISAfter(spec, fta, FORWARD, size);
        if (res) return res;
    }
}