...
namespace {
    struct MergeInfo {
        FTANode* merged;
        int y_index;
        MergeInfo(FTANode* _merged, int _y_index): merged(_merged), y_index(_y_index) {}
    };
}

namespace {
     void _mergeOnTrie(trie::TrieNode* node, int index, FixedVector<FTANode*>& tmp,
    ...
    std::vector<std::pair<FixedVector<FTANode*>, FTANode*>> mergeOnTrie(trie::TrieNode* root, const std::vector<std::vector<MergeInfo>*>& storage) {
...
PFTA fta::merge::mergeFTAForward(FTA *x, FTA *y, bool is_cut, TimeGuard* guard) {
    ...
    auto* grammar = x->grammar; grammar->indexSymbol();

    // auxiliary data structures
    std::vector<std::vector<MergeInfo>> merge_storage(x_size); // get refined nodes from x
    ...
    auto get_node = [&](FTANode* x_node, FTANode* y_node, std::vector<MergeInfo>& info_list) {
        assert(x_node->size == y_node->size && x_node->symbol == y_node->symbol);
        auto y_index = y_node->getInfo();
        auto* pre_node = new_node_holder[y_index];
        if (pre_node && pre_node->getInfo() == x_node->getInfo()) {
            return pre_node;
        }
        auto new_info = std::make_shared<BinaryOutputInfo>(x_node->oup_info, y_node->oup_info);
            ...
            for (auto* x_node: storage[size]) {
                ...
                for (auto* x_edge: x_node->edge_list) {
                    ...
                    if (guard && guard->isTimeout()) {
                        _clearInfo(merge_storage); return {};
                    }

                    // LOG(INFO) << "  Processing edge " << x_edge->toString();
                    std::vector<std::vector<MergeInfo>*> child_storage;
                    for (auto x_child: x_edge->node_list) {
                        child_storage.push_back(&merge_storage[x_child->getInfo()]);
                    }
                    auto edge_feature = trie::getEdgeFeature(x_node->symbol, x_edge->semantics.get());
                    auto it = y_edge_holder.find(edge_feature);
                    if (it == y_edge_holder.end()) continue;
                    // LOG(INFO) << edge_feature << " " << child_storage.size();
                    auto possible_edges = mergeOnTrie(it->second.get(), child_storage);
                    for (auto& [children, y_node]: possible_edges) {
                        auto base_node = get_node(x_node, y_node, merge_storage[x_node->getInfo()]);
                        auto* new_edge = new FTAEdge(x_edge->semantics, std::move(children));
    ...
    auto res = std::make_shared<FTA>(x->grammar, x->size_limit, holder, root_list, info_list);
...
namespace {
    ...
    typedef std::unordered_map<std::string, std::pair<std::vector<FTAEdge*>, std::vector<FTAEdge*>>> PairEdgeInfo;
    PairEdgeInfo _buildEdgeInfo(FTANode* x, FTANode* y) {
        PairEdgeInfo info;
        for (auto* edge: x->edge_list) info[edge->semantics->getName()].first.push_back(edge);
        for (auto* edge: y->edge_list) info[edge->semantics->getName()].second.push_back(edge);
        return info;
    }

    void _clearInfo(const std::vector<std::unordered_map<int, SplitInfo>>& info_holder) {
...
PFTA fta::merge::mergeFTABackward(FTA *x, FTA *y, bool is_cut, TimeGuard* guard) {
    int x_size = assignFTAIndex(x), y_size = assignFTAIndex(y);
    if (x_size < y_size) {
        std::swap(x_size, y_size); std::swap(x, y);
    }
#ifdef DEBUG
    ...
    auto get_merge_node = [&](FTANode* x_node, FTANode* y_node) -> FTANode* {
        ...
        if (it == holder.end()) {
            auto oup_info = std::make_shared<BinaryOutputInfo>(x_node->oup_info, y_node->oup_info);
            auto* new_node = new FTANode(x_node->symbol, oup_info, y_node->size);
    ...
    for (auto* x_root: x->root_list) {
        for (auto* y_root: y->root_list) {
            if (x_root->size == y_root->size && x_root->symbol->id == y_root->symbol->id) {
            ...
            for (auto* x_node: storage[size]) {
                for (auto& [_y_id, info]: split_holder[INDEX(x_node)]) {
                    ...
                    for (auto& [_rule_name, rules]: edge_info) {
                        // check timeout
                        if (guard && guard->isTimeout()) {
                            _clearInfo(split_holder); return {};
                        }
                        for (auto* x_edge: rules.first) {
                            for (auto* y_edge: rules.second) {
#ifdef DEBUG
                                assert(x_edge->semantics->getName() == y_edge->semantics->getName());
                                assert(x_edge->node_list.size() == y_edge->node_list.size());
#endif
                                FixedVector<FTANode*> child_list(x_edge->node_list.size());
                                for (int i = 0; i < x_edge->node_list.size(); ++i) {
                                    child_list[i] = get_merge_node(x_edge->node_list[i], y_edge->node_list[i]);
                                }
                                auto* new_edge = new FTAEdge(x_edge->semantics, std::move(child_list));
    ...
    auto res = std::make_shared<FTA>(x->grammar, x->size_limit, holder, root_list, concat(x->info_list, y->info_list));
    ...
    void _swapMerge(FTA* fta, int left_size) {
        ...
        for (auto& [_, holder]: fta->node_map) {
            for (auto& list: holder) {
                for (auto* node: list) {
                    auto* info = dynamic_cast<BinaryOutputInfo*>(node->oup_info.get());
                    assert(info);
                    auto new_info = std::make_shared<BinaryOutputInfo>(info->r, info->l);
...
PFTA fta::mergeFTA(FTA *x, FTA *y, MergeType type, bool is_cut, TimeGuard* guard) {
    PFTA result; bool is_swapped = false;
    if (x->nodeCount() < y->nodeCount()) {
        std::swap(x, y); is_swapped = true;
    }
    switch (type) {
        case FORWARD: {
...
PFTA fta::mergeIndependentFTAForward(const FTAList &fta_list, bool is_cut) {
    auto init = fta::grammar2FTA(fta_list[0]->grammar, fta_list[0]->size_limit, true);
    FTAList unit_list;
    for (auto& fta: fta_list) unit_list.push_back(fta::mergeFTA(init.get(), fta.get(), FORWARD, true));
