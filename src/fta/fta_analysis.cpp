...
void analysis::analyzeMerge(Grammar *grammar, Env *env, const IOExampleList &example_list, int size) {
    for (int n = 2; n <= 2; ++n) {
        ...
        }*/
        {
            ...
            int cost = 0;
            for (int i = 1; i < n; ++i) {
                auto next_left = buildFTA(grammar, env, example_list[i], size, true);
                fta_left = mergeFTA(fta_left.get(), next_left.get(), FORWARD, false);
                cost += fta_left->edgeCount();
                fta_left->cutBackward();
                auto next_right = buildFTA(grammar, env, example_list[i + n], size, true);
                fta_right = mergeFTA(fta_right.get(), next_right.get(), FORWARD, false);
                cost += fta_right->edgeCount();
                fta_right->cutBackward();
            }
            LOG(INFO) << "  left:  " << fta_left->getSizeInfo();
            LOG(INFO) << "  right: " << fta_right->getSizeInfo();
            auto forward_final = mergeFTA(fta_left.get(), fta_right.get(), FORWARD, false);
            LOG(INFO) << "  forward cost " << forward_final->edgeCount() + cost;
            LOG(INFO) << "    " << size::sizeInfo2String(size::getNodeCountVector(forward_final.get()));
            ...
            /*auto backward_final = mergeFTABackward(fta_left.get(), fta_right.get(), false);
            LOG(INFO) << "  backward cost " << backward_final->edgeCount() + cost;
            LOG(INFO) << "    " << size::sizeInfo2String(size::getNodeCountVector(backward_final.get()));
            ...
            /*auto backward_overlap = mergeFTABackwardWithOverlapElimination(fta_left.get(), fta_right.get(), false);
            LOG(INFO) << "  backward overlap cost " << backward_overlap->edgeCount() + cost;
            LOG(INFO) << "    " << size::sizeInfo2String(size::getNodeCountVector(backward_overlap.get()));
