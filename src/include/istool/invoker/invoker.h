...
class InvokeConfig {
    class InvokeConfigItem {
       public:
        ...
        InvokeConfigItem(void* _data,
                         std::function<void(void*)> _free_operator);
        ~InvokeConfigItem();
    };
   ...
   public:
    ...
    template <class T>
    void set(const std::string& name, const T& w) {
    ...
    template <class T>
    T access(const std::string& name, const T& default_w) const {
        auto it = item_map.find(name);
 ...
 *   Default: Always true
 */
Solver* buildOBE(Specification* spec, Verifier* v, ExampleList examples,
                 const InvokeConfig& config);
Solver* buildEuSolver(Specification* spec, Verifier* v,
                      const InvokeConfig& config);

/**
 * @config "prepare"
 *   Type: std::function<void(Grammar*, Env* env, const IOExample&)>
 * (VSAEnvSetter); Set constants that are used for building VSAs. Default: For
 * String, defined in executor/invoker/vsa_invoker.cpp
 *
 * @config "pruner"
 *   Type: VSAPruner* (defined in istool/ext/vsa/vsa_builder.h)
 *   An object that prunes off useless VSA nodes.
 *   Default: For String, keeps the first 1e5 VSA nodes and skip those nodes
 * that use stings longer than the input/output.
 *
 * @config "builder"
 *   Type: std::shared_ptr<VSABuilder>
 *   An object used to construct VSA from examples.
 *   Default: std::make_shared<BFSVSABuilder>(info->grammar, pruner,
 * spec->env.get(), prepare))
 *
 * @config "selector"
 ...
 *   Default: 7
 */
Solver* buildVanillaVSA(Specification* spec, Verifier* v,
                        const InvokeConfig& config);
 ...
 * @config "model"
 *   Type: TopDownModel*
 *   A cost model for programs in the program space. MaxFlash always returns the
 * solution with the minimum cost. Default: ext::vsa::getSizeModel()
 */
Solver* buildMaxFlash(Specification* spec, Verifier* v,
                      const InvokeConfig& config);
Solver* buildPolyGen(Specification* spec, Verifier* v,
                     const InvokeConfig& config);
Solver* buildCondSolver(Specification* spec, Verifier* v,
                        const InvokeConfig& config);
 ...
 * Default: 4
 */
Solver* buildExternalEuSolver(Specification* spec, Verifier* v,
                              const InvokeConfig& config);
Solver* buildExternalCVC5(Specification* spec, Verifier* v,
                          const InvokeConfig& config);
}  // namespace single

FunctionContext synthesis(Specification* spec, Verifier* v,
                          SolverToken solver_token, TimeGuard* guard,
                          const InvokeConfig& config = {});
std::pair<int, FunctionContext> getExampleNum(Specification* spec, Verifier* v,
