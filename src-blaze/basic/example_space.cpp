...
namespace {
    std::optional<bool> getBool(const Data &data) {
        auto *bv = dynamic_cast<BoolValue *>(data.get());
        return bv->w;
    }
} // namespace

ExampleSpace::ExampleSpace(const PProgram &_cons_program, Env *_env)
    : cons_program(_cons_program), env(_env) {}
bool ExampleSpace::satisfyExample(const FunctionContext &info,
                                  const Example &example) {
    try {
        auto res = env->run(cons_program.get(), example, info);
        auto res_bool = getBool(res);
        if (res_bool.has_value())
            return res_bool.value();
        else
            return false;
...
IOExampleSpace::IOExampleSpace(const std::string &_func_name)
...
void FiniteExampleSpace::removeDuplicate() {
    ...
    for (auto &example : example_space) {
        ...
        example_space[now++] = example;
    }
    example_space.resize(now);
}
FiniteExampleSpace::FiniteExampleSpace(const PProgram &_cons_program,
                                       const ExampleList &_example_space,
                                       Env *env)
    : ExampleSpace(_cons_program, env), example_space(_example_space) {}
FiniteIOExampleSpace::FiniteIOExampleSpace(const PProgram &_cons_program,
                                           const ExampleList &_example_space,
                                           const std::string &_name,
                                           const ProgramList &_inp_list,
                                           const PProgram &_oup, Env *env)
    : FiniteExampleSpace(_cons_program, _example_space, env),
      IOExampleSpace(_name), inp_list(_inp_list), oup(_oup) {}
...
bool FiniteIOExampleSpace::satisfyExample(const FunctionContext &ctx,
                                          const Example &example) {
    auto io_example = getIOExample(example);
    if (ctx.find(func_name) == ctx.end()) {
        LOG(FATAL) << "Cannot find program " << func_name;
    }
    return example::satisfyIOExample(ctx.find(func_name)->second.get(),
                                     io_example, env);
}
...
std::shared_ptr<FiniteIOExampleSpace>
example::buildFiniteIOExampleSpace(const IOExampleList &examples,
    ...
    auto cons_program =
        std::make_shared<Program>(env->getSemantics("="), sub_list);
    ...
    return std::make_shared<FiniteIOExampleSpace>(cons_program, example_list,
                                                  name, l_subs, r, env);
}

std::string example::ioExample2String(const IOExample &example) {
    return data::dataList2String(example.first) + "=>" +
           example.second.toString();
}

Example example::ioExample2Example(const IOExample &example) {
    Example res = example.first;
    res.push_back(example.second);
    return res;
}