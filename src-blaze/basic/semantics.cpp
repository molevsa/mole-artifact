...
XorSemantics::XorSemantics(): NormalSemantics("xor", TBOOL, {TBOOL, TBOOL}) {
}
Data XorSemantics::run(DataList &&inp_list, ExecuteInfo *info) {
    return Data(std::make_shared<BoolValue>(inp_list[0].isTrue() ^ inp_list[1].isTrue()));
}
...
void semantics::loadLogicSemantics(Env* env) {
    ...
    LoadSemantics("not", Not); LoadSemantics("xor", Xor);
}
