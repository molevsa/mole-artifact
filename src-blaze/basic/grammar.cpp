...
NonTerminal::NonTerminal(const std::string &_name, const PType &_type)
...
ConcreteRule::ConcreteRule(const PSemantics &_semantics,
                           const NTList &_param_list)
...
Grammar::Grammar(NonTerminal *_start, const NTList &_symbol_list,
                 bool is_remove_empty)
    : start(_start), symbol_list(_symbol_list) {
    int pos = -1;
...
namespace {
    bool _getSymbolDepth(NonTerminal *symbol,
        ...
        for (auto *rule : symbol->rule_list) {
            for (auto *sub : rule->param_list) {
                if (_getSymbolDepth(sub, cache, stack_set))
                    return true;
    ...
    int _getSymbolDepth(NonTerminal *symbol) {
        std::unordered_map<NonTerminal *, int> cache;
        std::unordered_set<NonTerminal *> stack_set;
        if (_getSymbolDepth(symbol, cache, stack_set))
            return 1e9;
        return cache[symbol];
    }
} // namespace
...
Grammar *grammar::generateHeightLimitedGrammar(Grammar *grammar, int limit) {
    grammar->indexSymbol();
    std::vector<NTList> symbol_pool(
        limit + 1, NTList(grammar->symbol_list.size(), nullptr));
    ...
    for (int h = 0; h <= limit; ++h) {
        for (int i = 0; i < grammar->symbol_list.size(); ++i) {
            ...
            auto *new_symbol = new NonTerminal(
                symbol->name + "@" + std::to_string(h), symbol->type);
            symbol_pool[h][i] = new_symbol;
            for (auto *rule : symbol->rule_list) {
                if (!h && !rule->param_list.empty())
                    continue;
                NTList sub_list;
...
std::string grammar::getFreeName(Grammar *grammar) {
    int id = 0;
    while (_isUsedName(grammar, "tmp@" + std::to_string(id)))
        ++id;
...
FullExecutedSemantics *grammar::getFullExecutedSemantics(Rule *rule) {
    auto *cr = dynamic_cast<ConcreteRule *>(rule);
    if (cr)
        return dynamic_cast<FullExecutedSemantics *>(cr->semantics.get());
    return nullptr;
}
...
ConstSemantics *grammar::getConstSemantics(Rule *rule) {
    auto *cr = dynamic_cast<ConcreteRule *>(rule);
    if (cr)
        return dynamic_cast<ConstSemantics *>(cr->semantics.get());
    return nullptr;
}
...
ParamSemantics *grammar::getParamSemantics(Rule *rule) {
    auto *cr = dynamic_cast<ConcreteRule *>(rule);
    if (cr)
        return dynamic_cast<ParamSemantics *>(cr->semantics.get());
    return nullptr;
}
...
PProgram grammar::getMinimalProgram(Grammar *grammar) {
    ...
    for (auto *symbol : grammar->symbol_list) {
        ...
        if (is_start)
            Q.push({d[symbol->id], symbol->id});
    }

    while (!Q.empty()) {
        auto k = Q.top();
        Q.pop();
        if (d[k.second] != k.first)
            continue;
...
int grammar::getMaxSize(Grammar *grammar) {
    ...
    for (int _ = 0; _ < n; _++) {
        ...
        for (auto *rule : finished_symbol->rule_list) {
            ...
            max_size_list[finished_symbol->id] =
                std::max(max_size_list[finished_symbol->id], total_size);
        }
    }
