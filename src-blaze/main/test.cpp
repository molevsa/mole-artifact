...
int main(int argv, char **argc) {
    ...
    if (z3_space) {
        ...
        auto fta_value_sets = std::make_shared<fta::value::FTAValueSets>();
        spec->example_space =
        ...
        for (int i = 0; i < example_num; ++i) {
            auto fta = fta::buildFTA(grammar, env, examples[i], start_size,
                                     true, fta_value_sets);
        ...
        for (int i = 1; i < example_num; ++i) {
            merged = fta::mergeFTA(merged.get(), base_list[i].get(),
                                   fta::FORWARD, true);
        ...
        for (int i = 1; i < share_num; ++i) {
            shared = fta::mergeFTA(shared.get(), base_list[i].get(),
                                   fta::FORWARD, true);
        }
        fta::FTAList unit_list;
        for (int i = share_num; i < base_list.size(); ++i) {
            auto unit = fta::mergeFTA(shared.get(), base_list[i].get(),
                                      fta::FORWARD, true);
            unit_list.push_back(unit);
        }
        LOG(INFO) << "Forward start" << std::endl;
        auto forward = fta::merge::mergeFTAForwardMulti(unit_list, false);
        LOG(INFO) << "Forward  Raw "
                  << fta::size::sizeInfo2String(
                         fta::size::getNodeCountVector(forward.get()));
        forward->fullyCut();
        LOG(INFO) << "Forward  Fin "
                  << fta::size::sizeInfo2String(
                         fta::size::getNodeCountVector(forward.get()));
        /*auto backward = fta::merge::mergeFTABackwardMulti(base_list, false,
        share_num); LOG(INFO) << "Backward Raw " <<
        fta::size::sizeInfo2String(fta::size::getNodeCountVector(backward.get()));
        backward->fullyCut();
        LOG(INFO) << "Backward Fin " <<
        fta::size::sizeInfo2String(fta::size::getNodeCountVector(backward.get()));*/

        LOG(INFO) << "Backward start";
        auto bid2 =
            fta::merge::mergeFTAStagedBidirectional(unit_list, false, 0.5);
        LOG(INFO) << "Bid Raw "
                  << fta::size::sizeInfo2String(
                         fta::size::getNodeCountVector(bid2.get()));
        bid2->fullyCut();
        LOG(INFO) << "Bid Fin "
                  << fta::size::sizeInfo2String(
                         fta::size::getNodeCountVector(bid2.get()));
    ...
    auto cegis_program =
        fta::synthesis::rawCEGIS(spec, fta::FORWARD, start_size);
    ...
    auto program = fta::synthesis::kFoldCEGIS(
        spec, fold_num, new fta::SizeExpectedScheduler(0.7, 3), start_size);
    ...
    auto fta = fta::synthesis::rawMerge(spec, global::example_recorder,
                                        fta::FORWARD, start_size);
    global::recorder.end("raw-merge");

    LOG(INFO) << "Program " << program->toString();
    LOG(INFO) << "RawMerge "
              << fta::util::extractMinimalProgram(fta.get())->toString();
