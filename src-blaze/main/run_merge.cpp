...
IOExampleList _getExamples(Specification *spec, int example_num,
                           int size_limit) {
    ...
    if (fio) {
        ...
        if (result.size() > example_num)
            result.resize(example_num);
        return result;
    }
    auto *zio = dynamic_cast<Z3IOExampleSpace *>(spec->example_space.get());
    if (zio) {
        auto fta =
            fta::grammar2FTA(spec->info_list[0]->grammar, size_limit, true);
        IOExampleList result =
            generateExamples(zio, example_num << 1, KIntRange, fta.get());
        if (result.size() > example_num)
            result.resize(example_num);
        return result;
    }
...
int main(int argv, char **argc) {
    ...
    if (argv > 1) {
        ...
        task_path = config::KSourcePath + "/tests/count1s.sl";
        exe_type = "fold4";
        example_num = 40;
        size = 27;
        init_num = 10;
    ...
    if (exe_type == "forward") {
        ...
        fta = fta::synthesis::kFoldMerge(spec, examples, fold_num,
                                         new fta::SizeExpectedScheduler(0, 2),
                                         size);
