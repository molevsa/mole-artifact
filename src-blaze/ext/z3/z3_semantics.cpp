...
Z3EncodeRes Z3XorSemantics::encodeZ3Expr(const std::vector<Z3EncodeRes> &inp_list) {
    auto x = inp_list[0], y = inp_list[1];
    return {x.res ^ y.res, merge(x.cons_list, merge(x.cons_list, y.cons_list))};
}
