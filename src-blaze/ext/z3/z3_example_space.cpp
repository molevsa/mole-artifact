...
Z3ExampleSpace::Z3ExampleSpace(
    const PProgram &_cons_prog, Env *_env, const TypeList &_type_list,
    const std::unordered_map<std::string, Signature> &_sig_map)
    : ExampleSpace(_cons_prog, _env), ext(ext::z3::getExtension(_env)),
      type_list(_type_list), sig_map(_sig_map) {}
Z3IOExampleSpace::Z3IOExampleSpace(const PProgram &_cons_prog, Env *_env,
                                   const TypeList &_type_list,
                                   const Signature &sig,
                                   const std::string &_name,
                                   const ProgramList &_inp_list,
                                   const PProgram &_oup_cons)
    : Z3ExampleSpace(_cons_prog, _env, _type_list, {{_name, sig}}),
      IOExampleSpace(_name), inp_list(_inp_list), oup_cons(_oup_cons) {}

bool Z3IOExampleSpace::satisfyExample(const FunctionContext &info,
    ...
    for (const auto &inp : inp_list)
        inp_vals.push_back(env->run(inp.get(), example));
    Data oup = env->run(p, example);
    auto full_inp = example;
    full_inp.push_back(oup);
    auto res = env->run(oup_cons.get(), full_inp);
    // LOG(INFO) << oup.toString() << " " << env->run(p->sub_list[0].get(),
    // full_inp).toString(); LOG(INFO) <<
    // env->run(p->sub_list[0]->sub_list[0].get(), full_inp).toString();
...
Example Z3IOExampleSpace::getInput(const Example &example) {
    ...
    for (int i = 0; i < inp_list.size(); ++i)
        res[i] = env->run(inp_list[i].get(), example);
    return res;
}
...
IOExample Z3IOExampleSpace::getIOExample(const Example &example) {
    ...
    z3::solver s(ext->ctx);
    auto encode_res = ext->encodeZ3ExprForProgram(
        oup_cons.get(), ext::z3::z3Vector2EncodeList(z3_inp_list));
    s.add(encode_res.res);
    s.add(z3::mk_and(encode_res.cons_list));
    auto res = s.check();
    if (res != z3::sat) {
        LOG(FATAL) << "Cannot find a valid output for input "
                   << data::dataList2String(example);
    }
...
PExampleSpace example::buildZ3ExampleSpace(
    const PProgram &cons, Env *env, const TypeList &type_list,
    const std::unordered_map<std::string, Signature> &sig_map) {
    std::unordered_map<std::string, ProgramList> invoke_map;
    collectAllInvokes(cons, invoke_map);
    if (invoke_map.size() != 1)
        return buildZ3ExampleSpace(cons, env, type_list, sig_map);
    std::string name;
    ProgramList invokes;
    ...
    return std::make_shared<Z3IOExampleSpace>(cons, env, type_list,
                                              sig_map.find(name)->second, name,
                                              inp_list, oup_cons);
}