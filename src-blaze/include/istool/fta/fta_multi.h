...
namespace fta {
    ...
    struct FoldInfo {
        ...
        void updateInfo();
        bool addExample(Env *env, const IOExample &example,
    ...
    namespace verify {
        bool verifyAfterExcludingExamples(const PProgram &program,
                                          Specification *spec,
    ...
    namespace kfold {
        namespace variants {
            FTAList
            adaptiveKFold(Specification *spec, int fold_num,
                          MultiMergeScheduler *scheduler, int size,
                          std::shared_ptr<value::FTAValueSets> fta_value_sets);
            FTAList adaptiveKFoldMulti(
                Specification *spec, int fold_num,
                MultiMergeScheduler *scheduler, int size,
                std::shared_ptr<value::FTAValueSets> fta_value_sets);
            FTAList prepareSharedUnitsMulti(
                Specification *spec, const IOExampleList &examples,
                int shared_num, int size,
                std::shared_ptr<value::FTAValueSets> fta_value_sets);
            FTAList prepareSharedUnits(
                Specification *spec, const IOExampleList &examples,
                int shared_num, int size,
                std::shared_ptr<value::FTAValueSets> fta_value_sets);
        } // namespace variants
        FTAList
        prepareUnits(Specification *spec, const IOExampleList &examples,
                     int shared_num, int size,
                     std::shared_ptr<value::FTAValueSets> fta_value_sets);
        FTAList kFold(Specification *spec, int fold_num,
                      MultiMergeScheduler *scheduler, int size,
                      std::shared_ptr<value::FTAValueSets> fta_value_sets);
    } // namespace kfold

    namespace synthesis {
        PProgram rawCEGIS(Specification *spec, MergeType type, bool merge_all,
                          int start_size = 1);
        ...
        PProgram kFoldCEGIS(Specification *spec, int fold_num,
                            MultiMergeScheduler *scheduler, bool merge_all,
                            int start_size = 1);
        PFTA kFoldMerge(Specification *spec, const IOExampleList &example_list,
                        int fold_num, MultiMergeScheduler *scheduler, int size);
