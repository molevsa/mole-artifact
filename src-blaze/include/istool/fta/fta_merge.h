...
namespace fta::merge {
    ...
    PFTA mergeFTABackwardMulti(const FTAList &fta_list, bool is_cut,
                               int share_num);
    PFTA mergeFTACostBasedBidirectional(const FTAList &fta_list, bool is_cut);
    PFTA mergeFTAStagedBidirectional(const FTAList &fta_list, bool is_cut,
                                     double alpha);
...
#define RemoveCopy(type)                                                       \
    ...
    namespace util {
        struct SplitHolder {
            std::map<FixedVector<FTANode *>, FTANode *,
                     FixedVectorCmp<FTANode *>>
                map;
            FTANode *base;

            SplitHolder(FTANode *_base);
            SplitHolder() = default;
            FixedVector<std::pair<FixedVector<FTANode *>, FTANode *>>
            moveOutInfoList();
        ...
        namespace backward {
            ...
            struct EdgeGroup {
                FTANode *node;
                std::vector<std::pair<EdgeFeature, FixedVector<FTAEdge *>>>
                    edge_groups;
                EdgeGroup();
            ...
            mergeEdgeGroup(const FixedVector<EdgeGroup *> &group_list);
            FTANodeList initRoots(FTA *base, const FTAList &other,
