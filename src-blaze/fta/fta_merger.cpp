...
namespace {
    ...
    std::vector<std::pair<FixedVector<FTANode *>, FTANode *>>
    mergeOnTrie(trie::TrieNode *root,
                const std::vector<std::vector<MergeInfo> *> &storage) {
...
PFTA fta::merge::mergeFTAForward(FTA *x, FTA *y, bool is_cut,
    ...
    for (auto *node : y->root_list)
        y_roots.insert(node->getInfo());
    ...
    auto res =
        std::make_shared<FTA>(x->grammar, x->size_limit, holder, root_list,
                              info_list, concat(x->examples, y->examples));
    if (is_cut)
        res->cutBackward();
    return res;
}
...
namespace {
    struct SplitInfo {
        ...
        SplitInfo(FTANode *_res, FTANode *_y_node)
            : res(_res), y_node(_y_node) {}
    };
...
PFTA fta::merge::mergeFTABackward(FTA *x, FTA *y, bool is_cut,
    ...
    auto res = std::make_shared<FTA>(
        x->grammar, x->size_limit, holder, root_list,
        concat(x->info_list, y->info_list), concat(x->examples, y->examples));
...
namespace {
    void _swapMerge(FTA *fta, int left_size) {
        ...
        for (int i = left_size; i < fta->info_list.size(); ++i) {
            ...
            examples.push_back(fta->examples[i]);
        }
