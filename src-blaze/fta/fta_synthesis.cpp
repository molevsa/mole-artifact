...
PFTA fta::synthesis::kFoldMerge(Specification *spec,
    ...
    auto full = fta::merge::mergeFTAForwardMulti(fta_list, false);
...
PFTA fta::synthesis::bidMerge(Specification *spec,
    ...
    auto unit_list = kfold::prepareUnits(spec, init_examples, config.shared_num,
                                         size_limit, fta_value_sets);
...
void fta::util::addExampleUsed(IOExample example) { _addExampleUsed(example); }

int fta::util::countExampleUsed() { return _countExampleUsed(); }