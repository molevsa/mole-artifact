...
struct CombineInfo {
    ...
    CombineInfo(CombineInfo &&other)
        : others(std::move(other.others)), children(std::move(other.children)) {
    }
...
namespace {
...
#ifdef DEBUG
        assert(others.size() == node_list.size() &&
               others.size() == result.size());
#endif
        for (int i = 0; i < others.size(); ++i) {
    ...
    void _mergeForward(int index, const std::vector<SplitHolder *> &info_holder,
        ...
        for (auto &[others, node] : info_list->map) {
            if (!_stepOnTrie(others, trie_nodes, current))
                continue;
            children[index] = node;
            _mergeForward(index + 1, info_holder, current, result, children,
                          sem);
        }
    }
    ...
    std::set<int> _getRootSet(FTA *fta) {
        ...
        for (auto *root_node : fta->root_list)
            root_set.insert(INDEX(root_node));
        return root_set;
    }
} // namespace
...
PFTA fta::merge::mergeFTAForwardMulti(const FTAList &fta_list, bool is_cut) {
    ...
    for (auto &fta : other_list)
        root_set.emplace_back(_getRootSet(fta.get()));
    ...
    for (auto *info : base->info_list)
        info_list.push_back(info);
    for (auto &other : other_list) {
        for (auto *info : other->info_list)
            info_list.push_back(info);
    }
    IOExampleList examples;
    for (auto example : base->examples)
        examples.push_back(example);
    for (auto &other : other_list) {
        for (auto example : other->examples)
            examples.push_back(example);
    }
    auto fta =
        util::createRawFTA(base, holder_list, root_list, info_list, examples);
    if (is_cut) {
        fta->cutBackward();
    }
    return fta;
...
void fta::merge::util::checkShared(const FTAList &fta_list) {
    for (auto &fta : fta_list)
        _checkShare(fta_list[0].get(), fta.get());
...
void EdgeGroup::init(FTANode *_node) {
    ...
    for (int now = 0, next = 0; now < edge_infos.size(); now = next) {
        ...
        for (int i = now; i < next; ++i)
            edges[i - now] = edge_infos[i]->second;
...
fta::merge::util::backward::mergeEdgeGroup(
    ...
    for (auto *group : group_list)
        assert(!group->edge_groups.empty());
#endif
...
FTANodeList
fta::merge::util::backward::initRoots(FTA *base, const FTAList &others,
...
namespace {
    void _combineEdge(int index, const EdgeStorage &storage,
        ...
        if (index == storage.size()) {
            ...
            for (int child_index = 0; child_index < holder_list.size();
                ...
                for (int edge_index = 0; edge_index < tmp.size();
                     ++edge_index) {
                    node_list[edge_index] =
                        tmp[edge_index]->node_list[child_index];
                }
    ...
    SmallVector<FixedVector<FTANode *>>
    _combineEdge(const EdgeStorage &storage, SplitHolderList &holder_list) {
#ifdef DEBUG
        assert(storage.size());
#endif
        SmallVector<FixedVector<FTANode *>> result(
            config::KDefaultSmallVecSize);
        for (auto *first_edge : storage[0]) {
            ...
            for (int i = 0; i < first_edge->node_list.size(); ++i) {
                base_holder_list[i] =
                    &holder_list[INDEX(first_edge->node_list[i])];
            }
...
PFTA fta::merge::util::createRawFTA(FTA *base,
                                    const SplitHolderList &holder_list,
                                    const FTANodeList &root_list,
...
PFTA fta::merge::mergeFTABackwardMultiWithShare(const FTAList &fta_list,
    ...
    IOExampleList examples;
    for (auto fta : fta_list) {
        for (auto example : fta.get()->examples) {
            examples.push_back(example);
        }
    }

    auto fta = util::createRawFTA(base, holder_list, roots,
                                  mergeInfoList(fta_list), examples);
    if (is_cut)
        fta->fullyCut();
    return fta;
}
