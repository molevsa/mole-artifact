...
namespace {
    struct GarbageInfo {
        ...
        FTANode *merge;
        GarbageInfo(FTANode *_base, FixedVector<FTANode *> &&_feature,
                    FTANode *_merge)
            : base(_base), feature(std::move(_feature)), merge(_merge) {}
    };

    void _bidForwardStep(FTANode *base,
                         const std::vector<trie::FTAEdgeHolder> &edge_holders,
                         SplitHolderList &holder_list,
                         std::vector<GarbageInfo> &garbage) {
        SplitHolder new_holder(base);
        fta::merge::util::forwardStep(base, edge_holders, holder_list,
                                      new_holder);
...
#ifdef DEBUG
        assert(trie_list.size() == new_trie_list.size() &&
               trie_list.size() == node_list.size());
#endif
    ...
    void _combineEdgeOnTrie(int index, const EdgeStorage &storage,
        ...
        for (auto *edge : storage[index]) {
            if (!_bidStepOnTrie(trie_list, new_trie_list, edge->node_list))
                continue;
            tmp[index - 1] = edge;
            _combineEdgeOnTrie(index + 1, storage, tmp, holder_list,
                               new_trie_list, result);
        }
    }

    SmallVector<FixedVector<FTANode *>>
    _combineEdgeOnTrie(const EdgeStorage &storage, SplitHolderList &holder_list,
        ...
        for (auto *first_edge : storage[0]) {
            ...
            _combineEdgeOnTrie(1, storage, tmp_list, base_holder_list,
                               trie_list, result);
        }
        return result;
    }

    void _bidBackwardStep(
        FTANode *base, const FixedVector<FixedVector<EdgeGroup>> &edge_holders,
        SplitHolderList &holder_list, FixedVector<PTrieNode> &root_list) {
        ...
        for (auto &[others, merged_node] : start.map) {
            ...
            for (auto &[sem, edge_storage] :
                 merge::util::backward::mergeEdgeGroup(group_list)) {
                for (auto &children :
                     _combineEdgeOnTrie(edge_storage, holder_list, root_list)) {
...
PFTA fta::merge::mergeFTACostBasedBidirectional(const FTAList &fta_list,
                                                bool is_cut) {
#ifdef DEBUG
    ...
    int n = ::util::assignFTAIndex(base);
    FTAList others;
    ...
    FixedVector<FixedVector<util::backward::EdgeGroup>> edge_group_list(
        fta_list.size());
...
#ifdef DEBUG
    for (auto &trie_root : split_trie_list)
        assert(!trie_root);
#endif
    ...
    while (forward_size < backward_size) {
        // LOG(INFO) << "forward: " << forward_size << "@" << forward_cost << ",
        // backward: " << backward_size << "@" << backward_cost;
        if (forward_cost < backward_cost) {
            // LOG(INFO) << "go forward";
            for (auto &[_, storage] : base->node_map) {
                for (auto *base_node : storage[forward_size]) {
                    _bidForwardStep(base_node, edge_holders, holder_list,
                                    garbage);
                    split_trie_list[INDEX(base_node)] =
                        _buildSplitTrie(holder_list[INDEX(base_node)]);
                }
            }
            forward_size++;
            ...
            for (auto &[_, storage] : base->node_map) {
                for (auto *base_node : storage[backward_size]) {
                    _bidBackwardStep(base_node, edge_group_list, holder_list,
                                     split_trie_list);
                }
            }
            backward_size--;
...
#endif
        holder_list[INDEX(info.base)].map.insert(
            std::make_pair(std::move(info.feature), info.merge));
    }
    ...
    auto fta = util::createRawFTA(base, holder_list, roots,
                                  util::mergeInfoList(fta_list), examples);
    if (is_cut)
        fta->fullyCut();
    return fta;
}

PFTA fta::merge::mergeFTAStagedBidirectional(const FTAList &fta_list,
                                             bool is_cut, double alpha) {
#ifdef DEBUG
    util::checkShared(fta_list);
#endif
    ...
    for (int size = 1; size <= mid_size; ++size) {
        ...
        for (auto &[_, storage] : base->node_map) {
            for (auto *base_node : storage[size]) {
                util::forwardStep(base_node, edge_holders, holder_list,
