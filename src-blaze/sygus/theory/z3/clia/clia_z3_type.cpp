...
Data Z3IntType::getValueFromModel(const z3::model &model, const z3::expr &expr,
                                  Type *type, bool is_strict) const {
...
z3::expr Z3IntType::buildConst(const Data &data, z3::context &ctx) const {
    return ctx.int_val(theory::clia::getIntValue(data).value());
}
z3::expr Z3IntType::buildVar(Type *type, const std::string &name,
...
Data Z3BoolType::getValueFromModel(const z3::model &model, const z3::expr &expr,
                                   Type *type, bool is_strict) const {
    auto value = model.eval(expr);
...
z3::expr Z3BoolType::buildVar(Type *type, const std::string &name,
                              z3::context &ctx) const {
