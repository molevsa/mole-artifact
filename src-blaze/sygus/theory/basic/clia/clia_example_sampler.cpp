...
ExampleList IntExampleGenerator::generateExamples(TimeGuard *guard) {
    Example res;
    int KIntMin = theory::clia::getIntValue(*int_min).value();
    int KIntMax = theory::clia::getIntValue(*int_max).value();
...
IntExampleGenerator::IntExampleGenerator(Env *_env, const TypeList &_type_list)
    : env(_env), type_list(_type_list) {
    int_min = env->getConstRef(theory::clia::KSampleIntMinName,
                               BuildData(Int, KDefaultIntMin));
    int_max = env->getConstRef(theory::clia::KSampleIntMaxName,
                               BuildData(Int, KDefaultIntMax));
}