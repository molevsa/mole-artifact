...
IntPlusSemantics::IntPlusSemantics(Data *_inf)
    : inf(_inf), NormalSemantics("+", TINT, {TINT, TINT}) {}
Data IntPlusSemantics::run(DataList &&inp_list, ExecuteInfo *info) {
    int inf_val = getIntValue(*inf).value();
    if (getIntValue(inp_list[0]).has_value() && getIntValue(inp_list[1])) {
        int w =
            getIntValue(inp_list[0]).value() + getIntValue(inp_list[1]).value();
        if (std::abs(w) > inf_val) {
            throw SemanticsError();
        }
        return Data(std::make_shared<IntValue>(w));
    } else {
        return Data(std::make_shared<IntValue>());
    }
}

IntMinusSemantics::IntMinusSemantics(Data *_inf)
    : inf(_inf), NormalSemantics("-", TINT, {TINT, TINT}) {}
...
IntTimesSemantics::IntTimesSemantics(Data *_inf)
    : inf(_inf), NormalSemantics("*", TINT, {TINT, TINT}) {}
...
IntDivSemantics::IntDivSemantics()
    : NormalSemantics("div", TINT, {TINT, TINT}) {}
Data IntDivSemantics::run(DataList &&inp_list, ExecuteInfo *info) {
    if (getIntValue(inp_list[0]).has_value() && getIntValue(inp_list[1])) {
        int x = getIntValue(inp_list[0]).value(),
            y = getIntValue(inp_list[1]).value();
        if (y == 0)
            throw SemanticsError();
...
IntModSemantics::IntModSemantics()
    : NormalSemantics("mod", TINT, {TINT, TINT}) {}
...
EqBoolSemantics::EqBoolSemantics()
    : NormalSemantics("=b", TBOOL, {TBOOL, TBOOL}) {}
...
IteSemantics::IteSemantics()
    : NormalSemantics("ite", TVARA, {TBOOL, TVARA, TVARA}) {}
