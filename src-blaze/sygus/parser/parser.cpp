...
namespace {
            ...
            for (auto &rule_node: nt_node[2]) {
                ...
                if (rule_node.isString()) {
                    ...
                    if (nt_map.find(rule_name) != nt_map.end()) {
                        symbol->rule_list.push_back(
                                new ConcreteRule(std::make_shared<DirectSemantics>(), {nt_map[rule_name]}));
                    } else {
                    ...
                    try {
                        ...
                        if (known_list.size() == param_list.size()) {
                            ...
                            symbol->rule_list.push_back(
                                    new _PartiallyKnownRule(s, std::move(known_list), std::move(param_list)));
                        }
                    }
                }
            }
    ...
    PProgram
    buildConsProgram(const Json::Value &entry, const std::map<std::string, std::pair<int, PType>> &var_info_map,
                     const std::map<std::string, PSynthInfo> &syn_info_map, std::map<std::string, PProgram> &cache,
                     Env *env) {
        ...
        if (syn_info_map.find(name) != syn_info_map.end()) {
            ...
            sem = std::make_shared<TypedInvokeSemantics>(syn_info->name, std::move(oup_type), std::move(inp_type_list),
                                                         env);
        } else {
...
namespace {
    ...
    void _loadBitVecLength(const Json::Value& root, Env* env) {
        ...
        if (size == -1) {
            theory::bv::setBitVectorLength(env, 1);
            LOG(INFO) << "BitVec length unknown, use the default value (1)";
        }
